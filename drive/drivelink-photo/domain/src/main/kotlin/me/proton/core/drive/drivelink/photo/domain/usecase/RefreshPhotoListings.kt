...
class RefreshPhotoListings @Inject constructor(
    private val photoRepository: PhotoRepository,
    private val photoListingAnchorRepository: PhotoListingAnchorRepository,
) {

    suspend operator fun invoke(userId: UserId, volumeId: VolumeId, tag: PhotoTag?): Result<Unit> = coRunCatching {
        photoRepository.deleteAll(userId, volumeId, tag)
        val key = tag?.let { PhotoListingAnchor.tagKey(it) } ?: PhotoListingAnchor.mainKey
        photoListingAnchorRepository.deleteAnchor(key, userId, volumeId)
    }
}
